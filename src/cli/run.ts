...
const runArchitecture = async (args: ParsedArgs): Promise<number> => {
  const { dir, config } = await resolveScanTarget(args);
  const report = await buildArchitectureReport(dir, { config });
  ...
  if (args.json) {
    ...
    process.stdout.write(renderArchitecture(report, { color: useColor(args) }));
  }
  // A cycle is a runtime hazard, not a style opinion — it fails the run.
...
const runReview = async (args: ParsedArgs): Promise<number> => {
  const { dir, config } = await resolveScanTarget({ ...args, positionals: [] });

  // Same change-set resolution as `impact`: an explicit path list, or a diff
  // scope (--diff <base> / --staged). Reusing it keeps the two commands
  // answering questions about exactly the same set of files.
  let files = (await resolveOnly(args, dir)) ?? [];
  if (args.positionals.length > 0) {
    // `computeImpact` keys on ABSOLUTE paths, so positionals are globbed the
    // same way `impact` does it rather than passed through as written.
    const fg = (await import("fast-glob")).default;
    for (const pattern of args.positionals) {
      const matched = await fg([pattern], { cwd: dir, absolute: true, dot: false, suppressErrors: true });
      files.push(...matched.filter((f) => SOURCE_EXT.test(f)));
    }
    files = [...new Set(files)];
  }
  if (files.length === 0) {
    process.stderr.write(
      "node-doctor review: name the changed files — a path, a glob, or a diff scope\n" +
        "  e.g. `node-doctor review src/db/pool.ts`, `node-doctor review --diff main`, or `--staged`.\n",
    );
    return 2;
  }

  const routing = await buildReviewRouting(dir, files, { config });

  if (args.json) {
    process.stdout.write(
      (args.jsonCompact ? JSON.stringify(routing) : JSON.stringify(routing, null, 2)) + "\n",
    );
    return 0;
  }
  process.stdout.write(renderReviewRouting(routing, { color: useColor(args) }));
  return 0;
};
...
/**
 * §159 — the shape of the change, not the quality of the code. Emits no
 * `Finding`; a shape is a reason to look, never a claim that something is
 * wrong, and the exit code reflects that.
 */
const runChangeShape = async (args: ParsedArgs): Promise<number> => {
  const { dir } = await resolveScanTarget(args);
  const report = await buildChangeShapeReport(dir, {
    base: args.diff || undefined,
    staged: args.staged,
  });

  if (args.json) {
    process.stdout.write((args.jsonCompact ? JSON.stringify(report) : JSON.stringify(report, null, 2)) + "\n");
    return report.available ? 0 : 2;
  }
  process.stdout.write(renderChangeShape(report, { color: useColor(args) }));
  // A diff that could not be read is a tool error; a clean diff is not.
  return report.available ? 0 : 2;
};

/**
 * §181 — locale integrity. Self-gating: a project with no locale files gets a
 * clean empty report rather than an error, because "this project does not
 * translate" is a perfectly good answer.
 */
const runI18n = async (args: ParsedArgs): Promise<number> => {
  const { dir, config } = await resolveScanTarget(args);
  // Without this, a typo'd path asserts "this project does not appear to be
  // translated" about a directory nobody ever read.
  if (!existsSync(dir) || !statSync(dir).isDirectory()) {
    process.stderr.write(`node-doctor i18n: "${args.positionals[0] ?? "."}" is not a directory\n`);
    return 2;
  }
  const report = await buildI18nReport(dir, { config });

  if (args.json) {
    process.stdout.write((args.jsonCompact ? JSON.stringify(report) : JSON.stringify(report, null, 2)) + "\n");
  } else {
    process.stdout.write(renderI18n(report, { color: useColor(args) }));
  }
  // A missing key or a broken placeholder is a user-visible defect and gates.
  // An unused key is housekeeping and does not.
...
export const main = async (argv: string[]): Promise<number> => {
  ...
  try {
    switch (args.command) {
      ...
      case "change-shape":
        return await runChangeShape(args);
      case "readiness":
