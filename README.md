...
Contributions are welcome — especially new rules and false-positive reports.
...
- Every rule ships with a valid/invalid test pair. The valid (does-not-fire) test
  is not optional.
- Precision over recall. When in doubt, stay silent. Add the ambiguous case to
  the good fixture and confirm it does not fire.
- Rules stay pure: no filesystem access, no host assumptions.
- Recommendations name the *specific mechanism* of the fix, not "sanitize input."
- Keep output deterministic.

**Reporting a false positive:** open an issue with the smallest source snippet
that triggers it and the rule id. False positives are treated as high-priority
bugs, because they are the thing most likely to get the tool uninstalled.

---

## Security policy

If you discover a security issue in node.doctor itself (as opposed to a rule you
would like added), please report it privately rather than opening a public issue.
See `SECURITY.md` for the disclosure process and contact.

node.doctor is a static analyzer: it reads code and never executes the code it
scans. It has no runtime access to the systems your code runs against.
...
**node.doctor** — because your agent writes bad Node, and someone should catch it.
